...
public class WidgetRestController {
    ...
    public ResponseEntity<Widget> createWidget(@RequestBody Widget widget){
        logger.info("Received widget: name: " + widget.getName() + ", description: " +
    ...
    public ResponseEntity<Widget> updateWidget(@RequestBody Widget widget,
                                               @PathVariable Long id, @RequestHeader(HttpHeaders.IF_MATCH)String ifMatch){
        //Get the widget with de specified id
        ...
        try{
            //Return a 200 response with the update widget
            return ResponseEntity
                    .ok()
                    .eTag(Integer.toString(widget.getVersion()))
